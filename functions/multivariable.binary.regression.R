...
do.multivariable.binomial.regression<- function(dataframe, outcome, independent.variable.list ) {
  ...
  left_join(predictor.largest.level)
    ...
    df <- df %>%
      ...
      select(outcome,outcome.reference,predictor,predictor.reference.level, predictor.level.tested,LowerBoundOR,OR, UpperBoundOR,OR.StdError,"p.value","Sig") 
    
    ## print to screen as html kable
    df %>%
      kbl(align='cccclcccccc',
          caption=paste("<b>Binomial logistic regression of multiple variables predicting",label(dataframe[[outcome]]),"</b>",sep=" ")) %>%
